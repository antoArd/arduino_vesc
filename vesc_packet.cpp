...
bool VescPacket::receive_packet(HardwareSerial *port, unsigned char *data)
{
  ...
  unsigned short crc_rx = (unsigned short)(rx_buffer[len + 2] = port->read()) << 8 | (unsigned short) port->read();
...
unsigned short VescPacket::crc16(unsigned char *buf, unsigned int len)
{
  ...
  for (i = 0; i < len; i++)
  {
    cksum = GetTableEntry(((cksum >> 8) ^ *buf++) & 0xFF) ^ (cksum << 8); //crc16_tab[(((cksum >> 8) ^ *buf++) & 0xFF)] ^ (cksum << 8);
  }
  return cksum;
}
